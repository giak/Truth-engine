...
10 CONSTRUCTION    Build FCT-001... from both search routes. Source:=SRC-ID+title/date+exact locator.
   ENFORCE[ANCHOR_OK]. Persisted exact excerpt supports only bounded fact "supplied content states X".
...
19a FACT_WRITEBACK For each current reopened revalidated ✦ only:
