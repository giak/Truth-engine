...
### 11.1 Six-Dimension EDI Calculation

**CRITICAL**: Replace vague "EDI:≥0.50" with EXACT calculated score "EDI:0.68".

```yaml
# EDI CALCULATION (6 Dimensions) - Compact DSL Notation

# D1: GEOGRAPHIC (weight 0.25)
continents: [  ]  # NA, EU, AS, AF, SA, OC (max 6)
zones: [  ]  # Specific regions (max 10)
local_present: false  # ≥1 source from affected region
geo_diversity = (len(continents)/6 × 0.4) + (min(len(zones)/10,1.0) × 0.3) + (local × 0.3)

# D2: LINGUISTIC (weight 0.20)
languages: [  ]  # EN, FR, ES, AR, RU, ZH, etc. (max 10)
non_english_count: 0  # Sources NOT in English
total_sources: 0
linguistic_families: [  ]  # Indo-European, Afro-Asiatic, Sino-Tibetan, etc. (max 5)
lang_diversity = (min(len(langs)/10,1.0) × 0.3) + (non_EN_count/total × 0.4) + (min(len(families)/5,1.0) × 0.3)

# D3: STRATIFICATION (weight 0.20)
primary_count: 0    # ◈
secondary_count: 0  # ◉
tertiary_count: 0   # ○
strat_diversity = (◈_pct × 0.5) + (◉_pct × 0.3) + (○_pct × 0.2)

# D4: OWNERSHIP (weight 0.15)
ownership_types: [  ]  # state, corporate, independent, academic, activist, personal (max 6)
non_corporate_count: 0  # state, independent, academic, activist, personal
ownership_diversity = (len(types)/6 × 0.6) + (non_corp_count/total × 0.4)

# D5: PERSPECTIVE (weight 0.15)
perspectives: [  ]  # ⟐, ⟐̅, 🌍, 🎓, 🔥 (max 7)
official_count: 0   # ⟐
counter_count: 0    # ⟐̅ + 🔥
dissident_present: false  # 🔥 exists
balance = 1.0 - abs(official_count - counter_count) / total
perspective_diversity = (len(perspectives)/7 × 0.5) + (balance × 0.3) + (dissident × 0.2)

# D6: TEMPORAL (weight 0.05)
temporalities: [  ]  # real_time, recent(<1w), medium(<1m), archival(>1y), historical (max 5)
archival_present: false  # ≥1 source >1 year old
temporal_diversity = (len(temporalities)/5 × 0.6) + (archival × 0.4)
...
# OUTPUT: EDI:{round(EDI, 2)}
```

**Expanded Template (if needed for complex calculations):** See backup kb/SEARCH_EPISTEMIC.md.backup_YYYYMMDD L976-1110 for detailed Python template with comments.
