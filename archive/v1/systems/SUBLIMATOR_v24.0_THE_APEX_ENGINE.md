# SUBLIMATOR v24.0 : THE ADAPTIVE ENGINE

**VERSION**: 24.0 : "The Adaptive Engine" (Generic & Iterative)
...
## §1 : CENSUS + DIGEST — PROTOCOLE AMÉLIORÉ

### §1.0 : INVENTAIRE PRÉALABLE (OBLIGATOIRE)

**AVANT toute extraction de faits, faire l'inventaire systématique.**

#### Step A: Découverte des investigations
1. `glob` sur le dossier investigations → lister tous les fichiers `*_INVESTIGATION.md`
2. Identifier les sous-dossiers
3. Déclarer "Date Système: [date]"

#### Step B: Extraction automatisée du comptage
**Utiliser ctx_execute pour compter les marqueurs ◈ et ◉:**

```javascript
const fs = require('fs');
const dir = 'CHEMIN_INVESTIGATIONS';
const files = fs.readdirSync(dir).filter(f => f.endsWith('_INVESTIGATION.md'));

let report = [];
files.forEach(f => {
  const content = fs.readFileSync(dir + '/' + f, 'utf-8');
  const confirmed = (content.match(/◈/g) || []).length;
  const probable = (content.match(/◉/g) || []).length;
  report.push({file: f, confirmed, probable, total: confirmed + probable});
});

console.table(report);
console.log('TOTAL:', report.reduce((a,b) => a + b.total, 0));
```

#### Step C: Identification des formats
Pour chaque investigation, identifier le format:
- **Format A**: Marqueurs ◈/◉ (extraction simple)
- **Format B**: Tables avec faits (parser les lignes)
- **Format C**: Narrative/texte (chercher patterns)

#### OUTPUT: Tableau d'inventaire
| Investigation | Format | ◈ | ◉ | Total |
|--------------|--------|---|---|-------|
...
1. Créer le dossier projet : `$INV/YYYY-MM-DD_<sujet>/`
2. Créer `sources/README.md` avec l'index des investigations
3. Reporter l'inventaire dans le dossier projet

---

### §1.2 : DIGEST — 3 ÉTAPES

**RÈGLE CARDINALE: ZÉRO OMISSION**

#### ÉTAPE 1: DECOUVRE
1. Lister TOUS les fichiers `*_INVESTIGATION.md` dans le dossier investigations
2. Pour chaque fichier, auto-détecter le type:
   - **MARQUE**: contient ◈ ou ◉
   - **TABLEAU**: contient des lignes avec | et faits structurés
   - **NARRATIF**: texte avec patterns "Fait:", "Fact:", ou numérotation

#### ÉTAPE 2: EXTRAIT
Pour chaque fichier, extraire selon son type:
- **MARQUE**: Extraire TOUTES les lignes contenant ◈ et ◉
- **TABLEAU**: Parser chaque ligne de fait vers format digest
- **NARRATIF**: Chercher "Fait:", "Fact:", ou patterns numériques

#### ÉTAPE 3: MERGE
1. Concaténer tous les faits extraits
2. Dédoublonner (garder une seule occurrence par fait identique)
3. Conserver les marqueurs ◈/◉ d'origine

**OUTPUT**: `00_DIGEST.md`

---

### §1.3 : CHECKPOINT #1 — VALIDATION QUANTITATIVE

**OBLIGATOIRE — Validation stricte**

```
〔VERIFICATION NEEDED #1 : Extraction Complète ?〕

### Inventaire initial (depuis §1.0)
- Investigations trouvées: {N}
- Faits totaux détectés (brut): {X}
  - ◈ CONFIRMÉ: {A}
  - ◉ PROBABLE: {B}

### Couverture par investigation
| Investigation | Faits source | Faits digest | Écart |
|--------------|---------------|--------------|-------|
| 01_xxx | X | Y | OK/MANQUE |
...

### Validation quantitative
□ Total digest = Total source ?
□ Aucun fait ◈ omis ?
□ Format B et C traités ?
□ Faits manquants identifiés ?
...
Travail **mécanique** : consolider les faits de tous les digests dans un tableau unique.

**OPÉRATIONS:**
1. Fusionner tous les faits des digests
2. Dédoublonner (même fait dans 2+ investigations → garder la version avec la meilleure source)
3. Résoudre les contradictions (même sujet, données différentes → signaler les deux versions)
4. Numéroter séquentiellement (F001, F002, ...)

**FORMAT**: Fichier `01_MATRICE.md`

| # | Fait | Date | Acteur(s) | Chiffre | URL source | Catégorie | Fiabilité | Source investigation |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|

**SECTION CONTRADICTIONS:**
```
### Contradictions détectées
| Fait A | Fait B | Nature | Résolution |
|--------|--------|--------|------------|
```
...
## §12 : CHANGEMENTS MAJEURS v22.0 → v24.0

| Version | Changements |
|---------|-----------|
| **v24.1** | **"The Zero-Omission Engine"** : Correction majeur du §1 Census+Digest. <br> - **§1.0 Inventaire Préalable** : Comptage automatisé ◈/◉ AVANT extraction. <br> - **§1.0 Step B** : Script ctx_execute pour quantification. <br> - **§1.0 Step C** : Identification des formats (A/B/C). <br> - **§1.2 Extraction Complète** : Format-aware extraction (◈/◉, tables, narrative). <br> - **§1.3 Checkpoint Quantitatif** : Validation stricte Total digest = Total source. |
...
*Version: 24.1 : "The Zero-Omission Engine"*
