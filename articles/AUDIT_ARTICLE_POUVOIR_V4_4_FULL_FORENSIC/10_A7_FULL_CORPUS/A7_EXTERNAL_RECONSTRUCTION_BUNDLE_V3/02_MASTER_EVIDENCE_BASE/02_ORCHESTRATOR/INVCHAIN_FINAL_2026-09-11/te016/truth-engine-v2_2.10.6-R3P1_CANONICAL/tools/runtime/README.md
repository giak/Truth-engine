# Truth Engine runtime 2.10.6 — bundle R3 forensic output contract
...
- `_NARRATIVE.tmp.md` frozen technical analytical body (historical filename retained for ABI stability)
...
- source family argument is bare `A|B|C|D|E|other:stable-token`, never `fam:A`. Every accepted source additionally records `title`, `canonical_id`, `publication_date`, `checked_at` and exact `locator`; omission blocks FINAL. `update-source` exists only to complete/repair truthful OPEN-state provenance, including bounded R2A.2 resume migration.
- semantic objects use `record-object` / `update-object`; postconditions may use `assert-counts`. Status-changing `update-object` requires `--reason`; EPI/tier-changing `update-fact` requires `--reason`; runtime derives `STATUS_DELTA_V1`.
- terminal semantic contracts are checked mechanically: SATURATED LED/AXS require non-empty `attempt_ids` + `result_ids`; terminal CLM requires claimant/materiality/support/counter/gap_type/gap/status; unresolved CAU requires typed gap.
- all non-derived report sections must be explicitly populated with content or `[]`; `NOT_INITIALIZED` blocks FINAL. `MODULE_EXECUTION` is a structured list covering conditionally loaded/materially executed modules. `MANIPULATION_REPORT` and `EDI_REPORT` are structured owner-contract objects, not prose summaries. Runtime-derived sections cannot be manually set.
...
- the technical body cannot contain `QRY-###`, JSON state dumps or renderer-owned forensic/registry headings; cite stable semantic/FCT/SRC identifiers. Historical narrative markers remain only as an ABI boundary.
- `set-persistence` accepts exactly one of `--json`, `--json-file`, `--stdin`.
- write the technical body through the historical `write-narrative` command; render/export use canonical paths by default and reject alternate paths.
...
PRE/DELIVERY finalization additionally requires all five ALWAYS LOAD modules recorded exactly once under canonical relative paths. Duplicate/aliased `truth-engine-v2/...` module paths are rejected.

### R3 deterministic forensic render

`render` composes the existing FINAL path only; it creates no additional artifact:

```text
RUN_MANIFEST
NARRATIVE_START/END (technical body)
FORENSIC_CONTRACT_V1
FORENSIC_SECTIONS_V1      # deterministic Markdown from report state
TRACE_MATRIX_V1           # derived from LED/AXS/CLM + evidence links
STATUS_DELTA_V1           # derived from observed status updates
OPEN_GAPS_V1              # derived typed gaps
SEMANTIC_REGISTRIES_V1
machine bookkeeping blocks
```

The deterministic Markdown renderer uses bullets/nested headings, not JSON dumps. JSON remains only inside runtime-owned semantic machine payloads where it is the canonical machine ABI.

### 2.10.6 persistence invariant

`set-persistence` MUST terminally account for every eligible fact before `export-snapshot`: either `success=1` with a real current memory id, or `blocked=1` with one canonical observed reason (`UNSTABLE_EVIDENCE_KEY`, `MNEMO_UNAVAILABLE`, `TOOL_SCHEMA_UNAVAILABLE`). In the `MNEMO_UNAVAILABLE` degraded branch the run must carry that degraded flag and the blocked FACT remains `mem:-`. DELIVERY rejects unresolved persistence, a missing local snapshot, successful writes without memory ids, and any snapshot/RUN_STATE memory-id mismatch. Source family tokens use one exact ABI end-to-end: `A|B|C|D|E|other:stable-token`.
Every `set-persistence` call is retained in `attempt_history` and rendered in `WRITEBACK_ATTEMPT_LOG_V1`; a later PASS does not erase an earlier FAIL/PARTIAL. DELIVERY requires the last attempt and the last corresponding `SYS PERSIST_REBIND` row to be PASS. Here PASS means terminally valid accounting, not that every eligible FACT was written to Mnemo.
