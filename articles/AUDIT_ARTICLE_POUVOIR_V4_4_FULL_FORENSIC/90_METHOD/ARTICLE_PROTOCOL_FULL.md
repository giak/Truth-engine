...
doc:
  ...
  status: c1r_structural_repair_candidate
foundation:
  ref: FOUNDATION.yml
summary: >
...
`HOLD` uniquement si une ambiguïté d'autorité ou de provenance empêche matériellement la tâche suivante. Une donnée simplement absente reste une limitation explicite.

### STOP

S01 s'arrête lorsque les objets matériels nécessaires à la tâche suivante ont une autorité bornée, que les conflits matériels sont résolus ou explicitement ouverts, et qu'aucune sélection éditoriale n'a été effectuée.
...
`HOLD` uniquement si une pièce connue comme matériellement nécessaire ne peut plus être récupérée et que cette perte empêche la tâche suivante. Une simple incertitude ou une absence non matérielle reste explicitée sans bloquer.

### STOP

S02 s'arrête lorsque le contexte actif est suffisant pour la tâche sans surcharge manifeste, que tout doute matériel détecté a été traité par récupération locale ou limitation explicite, que la matière exclue reste récupérable lorsqu'elle existe encore en amont, et qu'aucune sélection éditoriale n'a été effectuée.
...
`REPAIR` si la quintessence perd ou renforce un élément matériel alors que le brut est disponible. Réparer localement la quintessence, pas rejouer l'investigation.

### HOLD

`HOLD` seulement si une matière matérielle nécessaire est identifiée mais que son origine ne peut plus être inspectée suffisamment pour produire une quintessence honnête.

### STOP

S03 s'arrête lorsque le contrôle pré/post ne détecte plus de perte matérielle ni d'augmentation silencieuse de certitude, que la trace vers le brut reste praticable et qu'aucune sélection éditoriale n'a été effectuée.
---
...
`HOLD` seulement si la perte de mémoire/provenance empêche une décision matérielle et qu'aucun artefact disponible ne permet de la prendre honnêtement.

### STOP

S04 s'arrête lorsque les objets mémoire utilisés ne gagnent aucune autorité par répétition, que les chemins de retour nécessaires sont disponibles ou explicitement dégradés, que l'actualité n'est pas présumée et que MnemoLite indisponible n'entraîne ni fabrication ni blocage injustifié.
...
`REPAIR` si une assertion de compréhension :
...
La réparation porte d'abord sur la compréhension, pas sur les investigations amont.

### HOLD

`HOLD` seulement si la compréhension nécessaire à la poursuite d'A2 dépend d'une information matérielle inaccessible que la matière active et la récupération ciblée ne permettent pas de résoudre honnêtement.

### STOP

S01 s'arrête lorsque la compréhension est suffisamment formée pour poursuivre A2 : elle relie la matière, conserve les statuts et contre-hypothèses, rend les gaps visibles et reste non éditoriale.

`PASS S01` n'autorise pas A3. A2-S02 doit encore reconstruire la structure matérielle et A2-S03 challenger la compréhension avant la frontière éditoriale.
...
Réparer d'abord la structure de compréhension, sans réinvestigation globale.

### HOLD

`HOLD` seulement si une dimension matérielle nécessaire à la compréhension ne peut pas être reconstruite honnêtement depuis la matière active ni par récupération ciblée.

### STOP

S02 s'arrête lorsque les dimensions qui changent matériellement l'explication sont reconstruites au niveau nécessaire, que les relations restent calibrées, que contradictions et inconnues sont visibles et qu'une structure supplémentaire n'apporterait pas de gain de compréhension matériel identifié.

`PASS S02` n'autorise pas A3. A2-S03 doit encore challenger la suffisance de la compréhension et les besoins d'information.
...
`HOLD` seulement si un défaut matériel identifié empêche une compréhension honnête ou l'évaluation par A3 et qu'il reste non résolu après récupération locale, avec information nouvelle nécessaire. Le besoin est alors transmis à A6 ; A2 ne le résout pas par invention.

### STOP

S03 s'arrête lorsque tous les défauts matériels identifiés ont été corrigés, bornés comme limites explicites ou transmis comme besoins d'information nouvelle ; qu'aucun `NON_TROUVÉ` n'a été transformé en inexistence ; et que la compréhension peut être remise à A3 sans prétention d'exhaustivité.
...
`HOLD` seulement si la compréhension A2 est insuffisante pour identifier honnêtement le delta d'un candidat matériel et qu'une récupération locale ou un besoin d'information est réellement nécessaire. Une contribution faible n'est pas un HOLD.

### STOP

S01 s'arrête lorsque les candidats plausibles sont explicités avec leur delta, leur dépendance au rendement d'enquête, leurs bornes et leur statut de nouveauté candidate ; qu'aucun candidat n'exige une promotion épistémique ; et qu'aucune décision appartenant à S02-S04 ou A4 n'a été prise.
...
`HOLD` seulement si la relation au corpus est matériellement nécessaire pour la suite et que la mémoire éditoriale accessible est trop insuffisante pour distinguer honnêtement répétition, continuité ou nouveauté. L'incertitude peut aussi être transmise à S03/S04 lorsqu'elle ne bloque pas la décision.

### STOP

S02 s'arrête lorsque le gain central de chaque candidat a été comparé sur une surface éditoriale suffisante et bornée, que le delta restant est explicite, qu'aucune publication antérieure n'a été blanchie en preuve du fond et qu'aucune décision de S03-S04/A4 n'a été prise.
...
`HOLD` seulement si la promesse éditoriale dépend d'une ambiguïté matérielle que les sorties amont disponibles ne permettent pas de borner honnêtement. Un contrat non tenable n'est pas un HOLD : il est transmis comme tel à S04.

### STOP

S03 s'arrête dès qu'un contrat borné est établi ou que sa non-tenabilité est explicitement démontrée, avec aucune décision de forme et aucune mutation silencieuse des bornes A2/S02.
...
`HOLD` seulement si les sorties amont sont matériellement contradictoires au point d'empêcher de décider entre les quatre voies. Une simple préférence de format ou une possibilité d'amélioration n'est pas un HOLD.

### STOP

S04 s'arrête dès qu'une seule décision est matériellement justifiée, que les autres voies sont explicitement écartées dans l'état courant et qu'aucun plan, prose, `PUBLISH` ou BACK non autorisé n'a été produit.
...
`HOLD` seulement si une transition indispensable dépend d'une contradiction matérielle amont que les sorties A2/A3 disponibles ne permettent pas de borner sans changer le contrat. Une difficulté de structure ou une préférence narrative n'est pas un HOLD.

### STOP

S01 s'arrête lorsque la progression transmet le contrat avec toutes les obligations matérielles, que les transitions n'ajoutent aucune relation non établie, qu'aucune étape n'est justifiée seulement par la topologie du corpus et qu'aucune prose d'article n'a été produite.
...
`HOLD` seulement si une proposition indispensable à la progression ne peut pas être formulée honnêtement avec la matière récupérable et qu'une simple borne détruirait matériellement le contrat. Un manque de confort rédactionnel ou une citation difficile à placer n'est pas un HOLD.

#### STOP

S02 s'arrête lorsque chaque unité matérielle du draft possède un support et une portée adéquats ou une borne explicite, que les chemins de retour sont praticables, qu'aucune lignée n'est blanchie en corroboration et que la prose peut être transmise à S03 sans ajout matériel non tracé.
...
S04 s'arrête lorsque titre, chapô et conclusion ne dépassent pas le corps calibré, que les références sont cohérentes et qu'aucun nouveau BACK ou changement de thèse n'est introduit.
