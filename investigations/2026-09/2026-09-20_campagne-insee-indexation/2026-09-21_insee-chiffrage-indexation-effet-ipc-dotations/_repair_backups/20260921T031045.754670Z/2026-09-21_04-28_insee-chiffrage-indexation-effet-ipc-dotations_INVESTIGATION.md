...
COMPLEXITY:9→APEX | CHECKPOINT_SEQ:7 | LAST_COMPLETED:18b | NEXT_ACTION:NONE
...
Le gap de chiffrage hérité du run 17-04 (`CAU-002`, type `ACCESS`) est **traité et refermé en tant que question** : le coût d'un dixième de point d'inflation est chiffré, par canal, dans une publication de référence. Deux canaux sur cinq sont chiffrés : environ **0,5 Md€ de prestations sociales indexées** et environ **0,3 Md€ de charge d'intérêts de la dette** par dixième de point, sur une base d'environ **500 Md€** de prestations indexées (`FCT-009`). Les trois autres canaux — loyers révisés par l'IRL, dotations aux collectivités, masse salariale au SMIC — **ne sont pas chiffrés dans la documentation publique identifiée**, et ce run ne les comble pas par une estimation propre.
...
| Montant chiffré | Environ **500 Md€** de prestations sociales indexées ; environ **0,5 Md€** par dixième de point | `FCT-009` |
...
- `LED-001` — **Saturé.** Le gap de chiffrage hérité est traité par une publication de référence : 0,5 Md€ de prestations et 0,3 Md€ de charge d'intérêts par dixième de point.
...
- `FCT-009` — Coût par dixième de point : 0,5 Md€ de prestations, 0,3 Md€ de charge d'intérêts, base d'environ 500 Md€.
...
| Élément | Décision |
|---|---|
| `FCT-009` | **`RECHECK`.** Le chiffrage du coût d'un dixième de point est rouvert à la source, et non repris du parent. |
...
### CLAIM_REGISTRY_V1
CLM-001 | {"counter":"NONE_FOUND","gap":"","gap_type":"NONE","linked_leds":["LED-001","LED-007"],"proposition":"Un dixieme de point d inflation se transmet a environ 0,5 MdE de prestations sociales indexees et a environ 0,3 MdE de charge d interets de la dette, soit de l ordre de 0,8 MdE sur les deux canaux chiffres, pour une base d environ 500 MdE de prestations indexees.","status":"SUPPORTED","support":"FCT-009 (chiffrage de reference, famille E) ; coherence interne avec FCT-003 (revalorisation datee) et FCT-001/FCT-002 (canaux IRL et dotations non chiffres)"}
