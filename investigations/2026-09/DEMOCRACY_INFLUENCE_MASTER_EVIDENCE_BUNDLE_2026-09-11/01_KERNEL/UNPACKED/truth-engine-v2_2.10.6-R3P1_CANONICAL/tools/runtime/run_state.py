...
ALLOWED_WRITEBACK_BLOCK_REASONS = {"UNSTABLE_EVIDENCE_KEY", "MNEMO_UNAVAILABLE", "TOOL_SCHEMA_UNAVAILABLE"}
...
def load(path: str | Path) -> dict:
    ...
    # Bounded R2A.2 -> R3 OPEN-state normalization. Historical FINAL states are not recertified.
    if obj.get("run", {}).get("state") == "OPEN":
        obj.setdefault("status_updates", [])
        sec=obj.setdefault("sections", {})
        sec.setdefault("MODULE_EXECUTION", "NOT_INITIALIZED")
    return obj
...
def cmd_record_query(a):
    ...
    if a.accept_source:
        if not a.url or not a.role or not a.family or not a.canonical_id or not a.publication_date or not a.checked_at or not a.locator or not a.title:
            die("--accept-source requires --url --role --family --title --canonical-id --publication-date --checked-at --locator")
        if a.role not in SOURCE_ROLES:
            die("invalid source role")
        validate_family_token(a.family)
        for k in ("title","canonical_id","publication_date","checked_at","locator"):
            machine_field(k, getattr(a,k))
    out = {}
    def fn(s):
        src_id = "-"
        if a.accept_source:
            src_id = next_id(s, "SRC")
            src = {"id": src_id, "role": a.role, "family": a.family, "url": a.url, "title": a.title,
                   "canonical_id": a.canonical_id, "publication_date": a.publication_date, "checked_at": a.checked_at, "locator": a.locator}
...
def cmd_record_source(a):
    if a.role not in SOURCE_ROLES:
        die("invalid source role")
    validate_family_token(a.family)
    for k in ("url","title","canonical_id","publication_date","checked_at","locator"):
        machine_field(k, getattr(a,k))
    out = {}
    def fn(s):
        sid = next_id(s, "SRC")
        row = {"id": sid, "role": a.role, "family": a.family, "url": a.url, "title": a.title,
               "canonical_id": a.canonical_id, "publication_date": a.publication_date, "checked_at": a.checked_at, "locator": a.locator}
...
def cmd_update_source(a):
    payload=read_json_input(a)
    if not isinstance(payload,dict) or not payload:
        die("update-source payload must be a non-empty JSON object")
    allowed={"role","family","url","title","canonical_id","publication_date","checked_at","locator"}
    unknown=set(payload)-allowed
    if unknown: die("update-source unknown field(s): "+",".join(sorted(unknown)))
    if "role" in payload and payload["role"] not in SOURCE_ROLES: die("invalid source role")
    if "family" in payload: validate_family_token(payload["family"])
    for k,v in payload.items():
        if not isinstance(v,str): die(f"{k} must be a string")
        machine_field(k,v)
    def fn(s):
        src=source_by_id(s,a.id)
        if not src: die(f"unknown source: {a.id}")
        src.update(payload)
    mutate(a.state,fn)
    print(a.id)
...
def cmd_update_object(a):
    ...
    def fn(s):
        ...
        after=str(payload.get("status", before))
        if after != before and not a.reason:
            die("status-changing update-object requires --reason")
        row.update(payload)
        if after != before:
            machine_field("reason", a.reason)
            s.setdefault("status_updates", []).append({"id":a.id,"before":before or "NONE","after":after or "NONE","reason":a.reason})
    mutate(a.state, fn)
    print(a.id)
...
def cmd_update_fact(a):
    ...
    def fn(s):
        ...
        if origin not in {None, "", "-"} and origin == snapshot_memory_id:
            die("origin_memory_id must identify the prior fact, not the enclosing snapshot memory")
        before = f"{fact.get('epi','')}|{fact.get('tier','')}"
        after = f"{candidate.get('epi','')}|{candidate.get('tier','')}"
        if after != before and not a.reason:
            die("EPI/tier-changing update-fact requires --reason")
        reopen_for_repair(s, "FACT_REPAIR")
        fact.update(payload)
        fact["families"] = families
        if after != before:
            machine_field("reason", a.reason)
            s.setdefault("status_updates", []).append({"id":a.id,"before":before,"after":after,"reason":a.reason})
        sid = next_id(s, "SYS")
...
CANONICAL_GAP_TYPES = {"ACCESS","AUTHENTICITY","SCOPE","TEMPORAL","INDEPENDENCE","CONTRADICTION","CAUSALITY","RESPONSIBILITY"}
FINAL_CLM_STATUSES = {"SATURATED","PARTIAL","SUPPORTED","REFUTED","CONTRADICTED"}
...
def _explicit_none_or_nonempty(value) -> bool:
    if isinstance(value, str) and value.strip().upper() in {"NONE", "NONE_FOUND"}: return True
    return _nonempty(value)


def semantic_contract_errors(kind: str, row: dict) -> list[str]:
    """Deterministic FINAL-shape checks only; semantics remain KERNEL/KB-owned."""
    e=[]; oid=row.get("id", kind); st=str(row.get("status","")).strip().upper()
    def req(*names):
        for name in names:
            if name not in row or not _nonempty(row.get(name)):
                e.append(f"{oid} missing {name}")
    if kind=="LED":
        req("source_id","locator","lead","kind","materiality","routes","linked_ids","attempt_ids","result_ids","status")
        if str(row.get("materiality","")).upper() in {"DECISIVE","IMPORTANT"}: req("evidence_excerpt")
        if st in {"SATURATED","GAP"} and not _id_list(row.get("attempt_ids")): e.append(f"{oid} terminal LED lacks attempt_ids")
        if st=="SATURATED" and not _id_list(row.get("result_ids")): e.append(f"{oid} SATURATED LED lacks result_ids")
        if st=="GAP" and not gap_is_typed(row): e.append(f"{oid} GAP is untyped")
        if st=="EXCLUDED" and not any(str(x).startswith("EXCLUDE(") for x in (row.get("routes") or [])): e.append(f"{oid} EXCLUDED lacks EXCLUDE(reason) route")
    elif kind=="AXS":
        req("axis","question","sought_objects","links","attempt_ids","result_ids","status")
        if st in {"SATURATED","GAP"} and not _id_list(row.get("attempt_ids")): e.append(f"{oid} terminal AXS lacks attempt_ids")
        if st=="SATURATED" and not _id_list(row.get("result_ids")): e.append(f"{oid} SATURATED AXS lacks result_ids")
        if st=="GAP" and not gap_is_typed(row): e.append(f"{oid} GAP is untyped")
        if st in {"N/A","NA"} and not _nonempty(row.get("reason")): e.append(f"{oid} N/A lacks reason")
    elif kind=="CLM":
        req("claim","claimant","materiality","support","counter","gap_type","gap","status")
        if not _explicit_none_or_nonempty(row.get("support")): e.append(f"{oid} support must be explicit")
        if not _explicit_none_or_nonempty(row.get("counter")): e.append(f"{oid} counter must be evidence or NONE_FOUND")
        gt=str(row.get("gap_type","")).strip().upper()
        if gt in {"", "NULL", "UNSPECIFIED", "-"}: e.append(f"{oid} invalid gap_type")
        if gt!="NONE" and not _nonempty(row.get("gap")): e.append(f"{oid} gap_type={gt} lacks gap description")
        if gt=="NONE" and str(row.get("gap","")).strip().upper() not in {"NONE","N/A"}: e.append(f"{oid} no-gap claim must set gap=NONE")
        if st not in FINAL_CLM_STATUSES and st!="GAP": e.append(f"{oid} invalid terminal claim status {st or 'ABSENT'}")
    elif kind=="CAU":
        req("mechanism","support","counter","limit","status")
        if st in {"GAP","UNKNOWN","UNRESOLVED"} and not gap_is_typed(row): e.append(f"{oid} unresolved CAU requires typed gap")
        if st in {"SUPPORTED","SATURATED","REFUTED"} and not _nonempty(row.get("causal_right")): e.append(f"{oid} terminal CAU lacks causal_right")
    elif kind=="CTRL":
        req("control","support","status")
    elif kind=="ACT":
        req("action","actor","intent","support","status")
    return e


def source_contract_errors(src: dict) -> list[str]:
    e=[]; sid=src.get("id","SRC")
    for k in ("canonical_id","title","publication_date","checked_at","locator","url","role","family"):
        if not _nonempty(src.get(k)): e.append(f"{sid} missing {k}")
    return e


def canonical_module_errors(run: dict) -> list[str]:
    e=[]; mods=run.get("loaded_modules",[]) or []
    if len(mods)!=len(set(mods)): e.append("loaded_modules contains duplicates")
    for mod in mods:
        if not isinstance(mod,str) or not mod or mod.startswith("/") or "\\\\" in mod or mod.startswith("truth-engine-v2/") or ".." in Path(mod).parts:
            e.append(f"non-canonical loaded module path: {mod}")
    return e


def report_contract_errors(state: dict) -> list[str]:
    e=[]; sec=state.get("sections",{}); run=state.get("run",{})
    mr=sec.get("MANIPULATION_REPORT")
    if not isinstance(mr,dict):
        e.append("MANIPULATION_REPORT must be a structured object")
    else:
        required=("input_kind","mission_mode","symbol_stage","symbols","patterns","threats","rhetorical","complexity","clusters","implicit","speaker","assumptions","priorities","query_guidance")
        for k in required:
            if k not in mr: e.append(f"MANIPULATION_REPORT missing {k}")
        symbols=mr.get("symbols")
        if not isinstance(symbols,dict) or len(symbols)!=15: e.append("MANIPULATION_REPORT symbols must contain exactly 15 entries")
        elif any(str(v).upper().startswith("DEFERRED") for v in symbols.values()): e.append("MANIPULATION_REPORT contains DEFERRED final symbol")
        if str(mr.get("symbol_stage","")).upper()=="DEFERRED": e.append("MANIPULATION_REPORT symbol_stage remains DEFERRED")
    me=sec.get("MODULE_EXECUTION")
    if not isinstance(me,list):
        e.append("MODULE_EXECUTION must be a list")
    else:
        by={}
        for i,row in enumerate(me,1):
            if not isinstance(row,dict): e.append(f"MODULE_EXECUTION row {i} is not an object"); continue
            for k in ("module","trigger","reason","input_ids","operations_applied","result_ids","negative_results","not_computable","gaps","status"):
                if k not in row: e.append(f"MODULE_EXECUTION row {i} missing {k}")
            mod=row.get("module")
            if mod in by: e.append(f"MODULE_EXECUTION duplicate module {mod}")
            by[mod]=row
        conditional=[m for m in run.get("loaded_modules",[]) if m not in REQUIRED_ALWAYS_LOAD]
        miss=[m for m in conditional if m not in by]
        if miss: e.append("MODULE_EXECUTION missing loaded module(s): "+",".join(miss))
    edi=sec.get("EDI_REPORT")
    if not isinstance(edi,dict):
        e.append("EDI_REPORT must be a structured object")
    else:
        for k in ("source_counts","edi","dimensions","corpus","decisive_claim_coverage","diagnostic_not_truth"):
            if k not in edi: e.append(f"EDI_REPORT missing {k}")
        if edi.get("diagnostic_not_truth") is not True: e.append("EDI_REPORT diagnostic_not_truth must be true")
        dcc=edi.get("decisive_claim_coverage")
        if not isinstance(dcc,list): e.append("EDI_REPORT decisive_claim_coverage must be a list")
    return e


def final_contract_errors(state: dict) -> list[str]:
    e=[]
    e.extend(canonical_module_errors(state.get("run",{})))
    for kind in OBJECT_KINDS:
        for row in state.get(OBJECT_BUCKETS[kind],[]): e.extend(semantic_contract_errors(kind,row))
    for src in state.get("sources",[]): e.extend(source_contract_errors(src))
    e.extend(report_contract_errors(state))
    return e
...
def semantic_terminal(kind: str, row: dict) -> bool:
    ...
    if semantic_contract_errors(kind,row): return False
    ...
    if kind=="CLM": return st in FINAL_CLM_STATUSES or (st=="GAP" and gap_is_typed(row))
    if kind=="CAU":
...
def cmd_mark_final(a):
    def fn(s):
        ...
        contract_errors = final_contract_errors(s)
        if contract_errors:
            die("mark-final blocked by forensic contract: " + "; ".join(contract_errors))
        nonterminal=[]
...
def _normalized_block_reason(value: str) -> str:
    return str(value or "").strip().upper().replace("-", "_").replace(" ", "_")


def _terminal_block_row(s: dict, fact: dict, row: dict) -> bool:
    ...
    try:
        ...
        return False
    if (a, ok, fail, block) != (0, 0, 0, 1):
        return False
    if row.get("action") != writeback_action(fact):
        return False
    reason = _normalized_block_reason(row.get("reason"))
    if reason not in ALLOWED_WRITEBACK_BLOCK_REASONS:
        return False
    if fact.get("mem") not in {None, "", "-"}:
        return False
    if reason == "MNEMO_UNAVAILABLE":
        degraded = set(s.get("run", {}).get("degraded_flags", []))
        if "MNEMO_UNAVAILABLE" not in degraded:
            return False
        if "MNEMO_UNAVAILABLE" not in str(s.get("persistence", {}).get("mnemo_row", "")).upper():
            return False
    return True


def _fact_persistence_terminal(s: dict, fact: dict) -> bool:
    if not eligible_fact(fact):
        return True
    if fact.get("mem") not in {None, "", "-"}:
        return True
    row = next(
        (r for r in (s.get("persistence", {}).get("writeback_execution") or [])
         if isinstance(r, dict) and r.get("fct") == fact.get("id")),
        None,
    )
    return _terminal_block_row(s, fact, row)


def persistence_attempt_result(s: dict) -> str:
    """Result of the persistence *contract*, not a count of successful Mnemo writes.

    PASS means every eligible fact reached one valid terminal outcome:
      - successful write/update with a real memory id, or
      - explicitly blocked for one canonical observed reason.
    """
    p = s["persistence"]
    ...
    for fid, fact in eligible.items():
        ...
        try:
            ...
            return "PARTIAL"
        if any(x not in (0, 1) for x in (a, ok, fail, block)):
            return "PARTIAL"
        ...
        if block:
            if not _terminal_block_row(s, fact, row):
                return "PARTIAL"
        else:
            if ok != 1 or fact.get("mem") in {None, "", "-"}:
                return "PARTIAL"
            if _normalized_block_reason(row.get("reason")) not in {"", "NONE"}:
                return "PARTIAL"
...
def snapshot_alignment_errors(s: dict) -> list[str]:
    ...
    for f in s.get("facts", []):
        ...
        if eligible_fact(f) and expected in {None, "", "-"} and not _fact_persistence_terminal(s, f):
            errors.append(f"{f.get('id')} eligible fact has no terminal persistence before delivery")
...
def _md_scalar(value) -> str:
    if value is None: return "NONE"
    if isinstance(value,bool): return "true" if value else "false"
    return str(value).replace("\n"," ").strip() or "NONE"


def _render_md_value(value, level: int = 0) -> list[str]:
    indent="  "*level
    if isinstance(value,dict):
        out=[]
        if not value: return [indent+"- NONE"]
        for k,v in value.items():
            if isinstance(v,(dict,list)):
                out.append(indent+f"- **{k}:**")
                out.extend(_render_md_value(v,level+1))
            else: out.append(indent+f"- **{k}:** {_md_scalar(v)}")
        return out
    if isinstance(value,list):
        ...
        for i,v in enumerate(value,1):
            if isinstance(v,(dict,list)):
                out.append(indent+f"- **item {i}:**")
                out.extend(_render_md_value(v,level+1))
            else: out.append(indent+"- "+_md_scalar(v))
        return out
    return [indent+_md_scalar(value)]


def forensic_sections(state: dict) -> str:
    lines=["## FORENSIC_SECTIONS_V1"]
    for name in REQUIRED_REPORT_SECTIONS:
        value=state.get("sections",{}).get(name)
        lines += ["",f"### {name}"]
        if value in (None,"NOT_INITIALIZED",[]): lines.append("NONE")
        else: lines.extend(_render_md_value(value))
    return "\n".join(lines).rstrip()+"\n"


def _fact_query_ids(state: dict, fids: list[str]) -> list[str]:
    sids=[]
    for fid in fids:
        f=fact_by_id(state,fid)
        if f: sids.extend(f.get("sources",[]))
    sids=sorted(set(sids))
    qids=sorted({q.get("id") for q in state.get("requests",[]) if q.get("source") in set(sids) and q.get("id")})
    return qids + sids


def trace_rows(state: dict) -> list[dict]:
    out=[]
    for kind in ("LED","AXS","CLM"):
        for row in state.get(OBJECT_BUCKETS[kind],[]):
            support=_id_list(row.get("support"))
            counter=_id_list(row.get("counter"))
            attempts=_id_list(row.get("attempt_ids"))
            if kind=="CLM" and not attempts: attempts=_fact_query_ids(state,support+counter)
            results=_id_list(row.get("result_ids")) or support+counter
            out.append({"id":row["id"],"attempts":attempts,"support":support,"counter":counter,"results":results,"final":row.get("status",""),"gap_type":row.get("gap_type","NONE")})
    return out


def trace_blocks(state: dict) -> str:
    lines=["## TRACE_MATRIX_V1"]
    for r in trace_rows(state):
        fmt=lambda xs: ",".join(xs) if xs else "-"
        lines.append(f"{r['id']} | attempts:{fmt(r['attempts'])} | support:{fmt(r['support'])} | counter:{fmt(r['counter'])} | results:{fmt(r['results'])} | final:{r['final']} | gap:{r['gap_type']}")
    lines += ["", "## STATUS_DELTA_V1"]
    for i,row in enumerate(state.get("status_updates",[]),1):
        lines.append(f"DELTA-{i:03d} | {row['id']} | {row['before']} | {row['after']} | {row['reason']}")
    if not state.get("status_updates"): lines.append("NONE")
    lines += ["", "## OPEN_GAPS_V1"]
    gaps=derive_snapshot_gaps(state)
    if not gaps: lines.append("NONE")
    for g in gaps:
        lines.append(f"{g.get('id')} | {g.get('kind')} | {g.get('status')} | {g.get('gap_type')} | {_md_scalar(g.get('gap'))}")
    return "\n".join(lines).rstrip()+"\n"


def forensic_contract_row(state: dict) -> str:
    mr=state.get("sections",{}).get("MANIPULATION_REPORT") or {}
    me=state.get("sections",{}).get("MODULE_EXECUTION") or []
    edi=state.get("sections",{}).get("EDI_REPORT") or {}
    complete_sources=sum(1 for x in state.get("sources",[]) if not source_contract_errors(x))
    dcc=edi.get("decisive_claim_coverage",[]) if isinstance(edi,dict) else []
    return (f"FORENSIC_CONTRACT_V1:SYMBOLS:{len(mr.get('symbols',{})) if isinstance(mr,dict) else 0}"
            f"|MODULE_EXECUTION:{len(me) if isinstance(me,list) else 0}"
            f"|EDI_DECISIVE:{len(dcc) if isinstance(dcc,list) else 0}"
            f"|SRC_COMPLETE:{complete_sources}/{len(state.get('sources',[]))}")
...
def machine_blocks(state: dict, phase: str) -> str:
    ...
    for src in state["sources"]:
        lines.append(f"{src['id']} | {src['role']} | fam:{src['family']} | {src['canonical_id']} | {src['title']} | {src['publication_date']} | {src['checked_at']} | {src['locator']} | {src['url']}")
...
def cmd_write_narrative(a):
    ...
    if re.search(r"\bQRY-\d{3}\b", text):
        die("technical body must not contain volatile QRY IDs; cite stable FCT/SRC IDs instead")
    if "<!-- NARRATIVE_START -->" in text or "<!-- NARRATIVE_END -->" in text:
        die("technical body boundary markers are renderer-owned")
...
def cmd_render(a):
    ...
    if re.search(r"(?m)^\s*(?:ENGINE|GATE_STATUS_V1|PERSISTENCE_META)\s*[:|]", narrative):
        die("technical body contains machine-owned header/persistence fields")
    if re.search(r"(?mi)^#{1,6}\s+(?:REQUEST_LOG|EVIDENCE_REGISTRY|FCT_SOURCE_MAP_V1|REFUTATION_REGISTRY_V1|WRITEBACK_PLAN_V1|MEMORY_WRITE_MODE_V1|CHECKPOINT_LOG_V1|WRITEBACK_EXECUTION_V1|WRITEBACK_ATTEMPT_LOG_V1)\s*$", narrative) or "FACT_REGISTRY_V1" in narrative or "SECTION_STATUS_V1" in narrative:
        die("technical body contains machine-owned registry section")
    if re.search(r"(?mi)^#{1,6}\s+(?:LEAD_REGISTRY(?:_V1)?|CLAIM_REGISTRY(?:_V1)?|AXIS_REGISTRY(?:_V1)?|CAUSALITY_REGISTRY(?:_V1)?|CONTROL_REGISTRY(?:_V1)?|ACTION_REGISTRY(?:_V1)?|SEMANTIC_REGISTRIES_V1)\s*", narrative):
        die("technical body duplicates runtime-owned semantic registry; store objects in RUN_STATE")
    if re.search(r"(?mi)^#{1,6}\s+(?:FORENSIC_SECTIONS_V1|TRACE_MATRIX_V1|STATUS_DELTA_V1|OPEN_GAPS_V1)\s*$", narrative):
        die("technical body duplicates renderer-owned forensic projection")
    text = (
        manifest_lines(state) + "\n<!-- NARRATIVE_START -->\n" + narrative
        + "\n<!-- NARRATIVE_END -->\n\n" + forensic_contract_row(state) + "\n\n"
        + forensic_sections(state) + "\n" + trace_blocks(state) + "\n" + semantic_blocks(state) + "\n" + machine_blocks(state, phase)
    )
...
def cmd_export_snapshot(a):
    s = load(a.state)
    history = s.get("persistence", {}).get("attempt_history") or []
    if not history or history[-1].get("result") != "PASS":
        die("export-snapshot requires a terminal PASS persistence attempt")
    unresolved = [
        f["id"] for f in s.get("facts", [])
        if eligible_fact(f) and not _fact_persistence_terminal(s, f)
    ]
    if unresolved:
        die("export-snapshot requires terminal persistence for all eligible facts: " + ",".join(unresolved))
    snap = {
        "snapshot_schema": 1,
        "engine": ENGINE,
        "run_id": s["run"]["run_id"],
...
def state_errors(s: dict, phase: str | None = None):
    ...
    if phase in {"pre", "delivery"}:
        errors.extend(f"{phase}: {x}" for x in final_contract_errors(s))
        r=s["run"]
        ...
        if phase == "pre":
            ...
            for f in eligible:
                ...
                if not r:
                    ...
                    continue
                if r.get("action") != writeback_action(f):
                    errors.append(f"delivery writeback action mismatch {f['id']}")
                ...
                if r.get("blocked") == 1 and not _terminal_block_row(s, f, r):
                    errors.append(f"delivery invalid terminal block {f['id']}")
...
def build_parser():
    ...
    x.add_argument("--accept-source", action="store_true"); x.add_argument("--role"); x.add_argument("--family"); x.add_argument("--title"); x.add_argument("--canonical-id"); x.add_argument("--publication-date"); x.add_argument("--checked-at"); x.add_argument("--locator"); x.set_defaults(func=cmd_record_query)
    x = sub.add_parser("record-source"); x.add_argument("--state", required=True); x.add_argument("--role", required=True); x.add_argument("--family", required=True); x.add_argument("--url", required=True); x.add_argument("--title", required=True); x.add_argument("--canonical-id", required=True); x.add_argument("--publication-date", required=True); x.add_argument("--checked-at", required=True); x.add_argument("--locator", required=True); x.set_defaults(func=cmd_record_source)
    x = sub.add_parser("update-source"); x.add_argument("--state", required=True); x.add_argument("--id", required=True); x.add_argument("--json"); x.add_argument("--json-file"); x.add_argument("--stdin", action="store_true"); x.set_defaults(func=cmd_update_source)
    ...
    x = sub.add_parser("update-object"); x.add_argument("--state", required=True); x.add_argument("--id", required=True); x.add_argument("--reason"); x.add_argument("--json"); x.add_argument("--json-file"); x.add_argument("--stdin", action="store_true"); x.set_defaults(func=cmd_update_object)
    x = sub.add_parser("update-fact"); x.add_argument("--state", required=True); x.add_argument("--id", required=True); x.add_argument("--reason"); x.add_argument("--json"); x.add_argument("--json-file"); x.add_argument("--stdin", action="store_true"); x.set_defaults(func=cmd_update_fact)
