...
STATE_ID := SHA256 de l'état complet du chantier. Avec Git disponible, il
reste fondé sur HEAD + diff + fichiers non suivis. Sans Git, un fallback
déterministe hash le tree de fichiers hors métadonnées/caches volatils.
La topologie Git n'influence jamais le verdict.
...
def _filesystem_state_rows(cwd):
    """Fallback STATE_ID sans Git, hors métadonnées/caches volatils."""
    ...
    for base, dirs, files in os.walk(root):
        ...
        for name in sorted(files):
            ...
            if rel.startswith(".verify/") and rel not in {".verify/config.json", ".verify/config.example.json"}:
                continue
            try:
...
def compute_state_id(cwd):
    """STATE_ID complet; Git si disponible, filesystem déterministe sinon."""
    ...
    if head:
        ...
        canonical = "mode=filesystem\nfiles:\n" + "\n".join(_filesystem_state_rows(cwd))
        state_head = "NO_GIT"
    state_id = hashlib.sha256(canonical.encode("utf-8")).hexdigest()
    return state_id, state_head
...
def run_checks(cfg, root):
    ...
    if not head:
        results.append(_record("git", "PASS", "Git absent/non initialisé : non bloquant par contrat; STATE_ID filesystem fallback"))
...
def check_deliverable_timestamp(deliverable):
    """Valide l'horodatage naïf du nom sans dépendre du timezone du process.

    Le nom canonique ne sérialise aucun fuseau. Il est donc impossible de
    comparer honnêtement HH:MM à l'heure locale du processus. On vérifie la
    date/heure calendaire puis la borne physique maximale UTC+14 : un timestamp
    au-delà de `UTC now + 14h` est futur dans tous les fuseaux civils possibles.
    """
    import re
    ...
    latest_possible_local = now_utc + timedelta(hours=14)
    if ts > latest_possible_local:
        return (
            "FAIL",
            f"horodatage futur dans tous les fuseaux civils : {name} "
            f"({ts:%Y-%m-%d %H:%M}) > UTC+14 bound {latest_possible_local:%Y-%m-%d %H:%M}",
        )
    return "PASS", f"horodatage compatible avec fuseau non sérialisé (borne UTC+14) : {name}"
