...
### Structured FINAL state contract (R3)

`EDI_REPORT` stored in RUN_STATE is a structured object, not a prose summary or hand-counted paragraph:

```text
source_counts: {primary, secondary, other, total}
edi: {final, raw, penalties, flags}
dimensions: {geo, lang, strat, owner, persp, temp}
corpus: {direct_objects, counters, coverage, independence, circularity, edi_star}
decisive_claim_coverage: [
  {claim_id, direct_object, independent_families, credible_counter, freshness, gap_type}
]
diagnostic_not_truth: true
```

Counts must be derived from canonical SRC/FCT/CLM registries whenever derivable. `diagnostic_not_truth` is literal true. Missing dimensions are explicit `N/A(reason)` under this module's rules, never silently omitted. The renderer may display a compact Markdown view, but RUN_STATE keeps this structure for deterministic validation.
