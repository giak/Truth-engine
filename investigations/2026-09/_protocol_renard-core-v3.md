...
> Protocole épistémique de recouvrement quand les preuves manquent ou que les pistes/indices sont insuffisants.
> Usage dans la campagne 2026-09 : cf. `_plan_investigations_absences-de-preuves.md` §0/§3 — bascule **obligatoire** dès qu'un atom n'a ni preuve probatoire ni piste exploitable.
> Compatibilité : RENARD renforce la discipline de recherche/construction d'une investigation KERNEL ; il ne remplace jamais KERNEL (RUN_STATE, gates PRE/DELIVERY via `tools/verify/verify.py` et FILE `_INVESTIGATION.md` restent contractuels).
...
AXIS := question + distinct test/surface + outcome that changes/refutes ; prioritize impact>evidence_family_diversity>source_accessibility>narrative_divergence ; open multiple axes if useful, rank, kill redundancy/low value ; diversify evidence families.
