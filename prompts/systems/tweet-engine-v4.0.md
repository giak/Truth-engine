...
**Architecture:** 6 layers, 3 user checkpoints, hook-first design, auto-save.
...
**Trigger:** After user approval (Y at final checkpoint)

### File Naming Convention

```yaml
PATTERN: logs/{date}_tweet_{subject}_{MODE}.md

COMPONENTS:
  date: YYYY-MM-DD (from investigation date)
  subject: Extracted from investigation filename, slugified
    - Remove date prefix
    - Convert spaces to hyphens
    - Lowercase
  MODE: MICRO | MEDIUM | LONG

EXAMPLES:
  - logs/2025-11-25_tweet_reichstadt-refus-debat_MEDIUM.md
  - logs/2025-11-25_tweet_fnsea-double-discours_LONG.md
  - logs/2025-11-25_tweet_budget-2025_MICRO.md
```
...
**Date**: {YYYY-MM-DD}
**Source**: {source_investigation_path}
**Mode**: {MICRO|MEDIUM|LONG}
**Hook Formula**: {FORMULA_NAME} ({trigger reason})
...
| Sections | {count} |
| Faits vérifiés | {X}/{total} |
| EDI source | {EDI from investigation} |
...
*Generated by Tweet Engine v4.0 | Source: Truth Engine v{version}*
```
...
1. GENERATE filename:
   - Extract date from investigation
   - Slugify subject from investigation filename
   - Append mode (MICRO/MEDIUM/LONG)
...
3. CONFIRM to user:
   "Tweet sauvegardé: logs/{filename}.md"

4. OPTIONAL: Offer to commit
   "Commit ce tweet? (Y/n)"
```
