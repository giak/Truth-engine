"""
Audit automatisé Phase 1 Sublimator (prompt-v36.md) sur n=12 quintessences (pilote août 2026).

Périmètre : investigations/2026-08/2026-08-13_corpus-investigations/_quintessence/
...
# 9 noms H2 canoniques (extraits verbatim de prompt-v36.md)
CANONICAL_H2 = [
...
SECTION_9_VARIANTS = [
    ...
    "9. Limites connues de cette extraction (case-limites) — [canonical]",
]

EM_DASH_BYTES = b"\xe2\x80\x94"

DEFAULT_QUINTESSENCE_DIR = Path("investigations/2026-08/2026-08-13_corpus-investigations/_quintessence")
...
F_ID_REGEX_EXTENDED = r"FCT-[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*"
...
def compute_delta_source_quintessence(quintessence_path: Path, text: str) -> dict:
    ...
    # Tolérance v2026-08-13 (corpus 120, après analyse des 44 faux positifs) :
    # 1) PREFIXE DE GROUPE : la quintessence référence le préfixe de groupe
    #    (ex. FCT-ppe2) en plus des faits individuels (FCT-ppe2-001..008).
    #    Un FCT-Q est satisfait si la source contient FCT-Q-<suite>.
    # 2) RENVOI INTER-DOSSIER : la quintessence peut citer un FCT présent dans
    #    un fichier frère _INVESTIGATION.md du même dossier (ex. note-axe1
    #    cite FCT-001 qui vit dans le fichier frère 13-43).
    # La vraie fabrication (FCT absent de la source ET de ses frères ET sans
    #   préfixe de groupe) reste détectée en ❌.
    sibling_f_ids: set[str] = set()
    if source_path.parent.is_dir():
        for sibling in sorted(source_path.parent.glob("*_INVESTIGATION.md")):
            if sibling.resolve() != source_path.resolve():
                sibling_f_ids |= extract_source_f_ids(sibling)

    def _fct_satisfied(fid: str) -> bool:
        if fid in source_f_ids:
            return True
        # préfixe de groupe : FCT-ppe2 satisfait par FCT-ppe2-001 dans la source
        if any(s.startswith(fid + "-") for s in source_f_ids):
            return True
        # renvoi inter-dossier : présent dans un fichier frère
        if fid in sibling_f_ids:
            return True
        return False

    fabricated = sorted(f for f in quintessence_f_ids if not _fct_satisfied(f))
...
def audit_file(path: Path) -> FileAudit:
    ...
    f_ids = re.findall(r"FCT-[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*", text)
