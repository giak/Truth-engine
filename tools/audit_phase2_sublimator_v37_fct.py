#!/usr/bin/env python3
"""
audit_phase2_sublimator_v37_fct.py
================================
...
Usage :
    python3 tools/audit_phase2_sublimator_v37_fct.py <rapport_synthese_phase2.md>
...
def parse_quintessence_source(source_dir: Path) -> dict:
    ...
    pattern_F = re.compile(r'FCT-[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*')
...
def parse_rapport(rapport: Path, source: dict) -> dict:
    ...
    pattern_cite = re.compile(r'FCT-[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*|M[1-4]')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 tools/audit_phase2_sublimator_v37_fct.py <rapport.md>")
        sys.exit(2)
    ...
    if not source_dir.exists():
        source_dir = rapport.parent.parent / "_quintessence"
