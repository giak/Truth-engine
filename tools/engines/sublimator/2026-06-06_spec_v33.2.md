...
**Note critique** : « LLM » désigne ici le LLM hôte (l'agent conversationnel opencode qui dialogue avec l'utilisateur), **PAS** une API externe. Aucune clé API n'est requise. Le pipeline mêle Python pur (mécanique) et lecture cursive par le LLM hôte.

- **Agent A — Script Parseur (Python pur)** : regex 3 formats + patterns article-utile (dates, sommes, %, citations, acteurs, sources) → JSON candidats + extractions atomiques
- **Agent B — Lecteur Cursif (LLM hôte)** : lit l'enquête intégralement et produit 11 sections de quintessence (thèse centrale, thèses implicites, acteurs, causalités, perspectives dialectiques, limites, wolves, iceberg, chronologie, domaines, URLs)
- **Agent C — Curator (Python pur)** : fusion candidats A + ajouts B, dédoublonnage (Jaccard < 0.7), scoring ✦/✧/⁅/❧
- **Agent D — Verifier (LLM hôte)** : relit l'enquête, signale F### manquants, incohérences, acteurs oubliés, fausses URLs, iceberg sous-exploité
- **Agent E — Humain** : valide ou refuse la fiche, avec un maximum de 2 boucles curator

### §X.8 Patterns article-utile (extractibles mécaniquement)

Pour qu'un article Substack soit solide, il a besoin de :

1. **Faits atomiques (F###)** : claims précis avec source
2. **Dates** : ISO (2024-01-15), français (15 janvier 2024), historiques (-2400, 326 av. J.-C., IIIe siècle), relatives (il y a 5000 ans)
3. **Sommes** : €, $, Mds, M, k, %, ratios
4. **Citations directes** : guillemets français « ... », avec auteur
5. **Noms propres** : PERSONNES (Acteurs, Wolves), LIEUX (toponymes), OUVRAGES, ORGANISATIONS
6. **URLs** : sources primaires (tier 1), secondaires (tier 2), cassées (tier 3)
7. **Marqueurs rhétoriques** : DEM (démagogie), BF (biais framing), NUM (chiffres avancés), AUTH (appel autorité), FAC (faits contrefaits)
8. **Symboles archétypaux** : Ξ€ΛΩΨ↕ΦΣΚ (registre symbolique SUBLIMATOR)
9. **Marqueurs de causalité** : « parce que », « donc », « car », « ainsi », « par conséquent », « ce qui implique »
10. **Sections enquête** : §1 RÉSUMÉ, §18 ICEBERG, §10 CHAÎNES, etc. (18 sections du plan SUBLIMATOR)

Tous ces patterns sont extractibles par regex Python pur. Agent A les produit ; Agent B (LLM hôte) les agrège en prose pour l'article.
