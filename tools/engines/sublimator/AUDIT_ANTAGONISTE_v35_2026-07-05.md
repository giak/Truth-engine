...
- [x] V1 — **Clos via ajout Note terminologique versions en tête de prompt-v35.md** (commit `0e29c8f`). Vérif forensique 2026-07-05 : `grep -nE 'v35|v36|v2|v3' prompt-v35.md prompts/quintessence_*.md` distingue clairement v35 (infra) / v36 (schéma quintessence) / §13.x (protocoles) / v2 (itération EXTRACTEUR post-§13.5 NO-GO). Le panachage est gorgoélienement supprimé dans le pilote et chaque sub-prompt utilise le label approprié à son rôle. **Auditabilité** : `git log -S 'Note terminologique versions' -- tools/engines/sublimator/prompt-v35.md` retrouve le commit d'introduction.
- [x] V2 — **Clos via réécriture §Phase 1 dans prompt-v35.md** (commit post-audit, ex. `0e29c8f` ou antérieur). Vérif forensique 2026-07-05 : la cross-référence `→ Voir ## Orchestration` est désormais **structurellement séparée** du blockquote « Format » par une ligne vierge, plus d'insertion au milieu d'une phrase. L'orphelin `(JSON par défaut…)` est rattaché au format canonique non sectionnement fautif. Phrase orpheline corrigée. **Auditabilité** : `git log -G 'JSON par défaut' -- tools/engines/sublimator/prompt-v35.md` retrouve les commits modifiant la zone §Phase 1.
- [x] V3 — **Clos via réécriture §Phase 2 dans prompt-v35.md** (commit post-audit, ~`0e29c8f`). Vérif forensique 2026-07-05 : la chaîne `\n\n` littérale (artefact caractérisant un échec de `str_replace` lors du refactor initial) est absente — `grep -nE '\\n\\n' prompt-v35.md` = 0 hit. La cross-référence `→ Voir ## Orchestration` est séparée par ligne vierge. Format `synthese_clusters.json + synthese.json` proprement rattaché. **Auditabilité** : `git log -G 'synthese_clusters' -- tools/engines/sublimator/prompt-v35.md` confirme la modification de la zone §Phase 2.
- [x] V4 — **Clos via sed** (commit `a7adbd7` selon audit). Vérif forensique 2026-07-05 : `grep -nE 'spanw' prompt-v35.md` = 0 hit. Le verbe `spawn` est correctement orthographié dans §Orchestration sub-section Étape D (dispatch ORCHESTRATEUR). **Auditabilité** : `git log -S 'spanw' --diff-filter=D -- tools/engines/sublimator/prompt-v35.md` retrouve le commit où `spanw` a été remplacé par `spawn`.
...
- [x] V8 — **Clos via sed** (commit `a7adbd7` selon audit). Vérif forensique 2026-07-05 : `grep -nE 'défini défini' prompts/validation_3enquetes.md` = 0 hit. Le mot `défini` apparaît **une seule fois** par agent dans §4.2 (LECTEUR, EXTRACTEUR, CRITIQUE, ORCHESTRATEUR). Plus de redoublement littéral. **Auditabilité** : `git log -S 'défini défini' --diff-filter=D -- tools/engines/sublimator/prompts/validation_3enquetes.md` retrouve le commit dédoublonnage.
...
- [x] V11 — **Clos via ajout d'une section « Statut Phase 0 » + amendement enum `status`** dans prompt-v35.md (commits `0e29c8f`). Vérif forensique 2026-07-05 : `grep -nE '\"status\"' prompt-v35.md` montre enum `\"status\": \"ok | needs_llm | llm_filled | error\"` (4 valeurs, dont `error` ajoutée par code-reviewer micro-obs post-commit). Section Phase 0 spécifie 3 sorties : OK (mode dégradé toléré) / error (HALTE inconditionnel) / needs_llm (complétion manuelle). **Auditabilité** : `git log -S 'ok | needs_llm | llm_filled | error' -- tools/engines/sublimator/prompt-v35.md` retrouve le commit d'introduction de l'enum complet.
- [x] V12 — **Clos via ajout `search_mode="hybrid"` × 4 sub-prompts** (commit `a7adbd7` selon audit). Vérif forensique 2026-07-05 : `grep -nE 'search_mode' prompts/quintessence_{reader,extractor,critic,orchestrator}.md` = 4 hits (1 par sub-prompt), chacun avec consigne explicite « ne jamais sans `search_mode="hybrid"` ». Le risque de TAG-ONLY retombé en silence est éliminé. **Auditabilité** : `git log -S 'search_mode="hybrid"' -- tools/engines/sublimator/prompts/quintessence_*.md` retrouve le commit d'introduction.
- [x] V13 — **Clos via ajout `get_system_snapshot` + règle HALTE en tête × 4 sub-prompts** (commit `a7adbd7` selon audit). Vérif forensique 2026-07-05 : `grep -nE 'get_system_snapshot' prompts/quintessence_{reader,extractor,critic,orchestrator}.md` = 4 hits. Consigne `> Étape 0 : get_system_snapshot. Si DOWN ⇒ HALTE` propagée uniformément. Récurrence confirmé : l'orchestrateur §Étape D précise correctement que la résilience interne (`giveup`) locale ne déclenche pas de HALTE global sur le pipeline. **Auditabilité** : `git log -S 'get_system_snapshot' -- tools/engines/sublimator/prompts/quintessence_*.md` retrouve le commit d'introduction.
...
- [x] V16 — **Clos via option (b) compress_summary** (commit post-audit, ~round `64389da`). L'audit V16 §2 autorisait « matérialiser dans `sublimator:iterations:<enquete_id>` Mnemolite OU dans `compress_summary` de la quintessence » (option OU). L'option (b) a été retenue : `iteration_count` et `iteration_alert` sont matérialisés dans prompt-v35.md §Phase 1.5 + quintessence_extractor.md §10 + quintessence_orchestrator.md. `iteration_alert: true` se déclenche à ≥ 2 itérations sans EXCELLENT. La clé Mnemolite `sublimator:iterations:<enquete_id>` (option a) n'est PAS matérialisée. **Décision 2026-07-05** : retenir l'option (b) — conforme à l'intent audit, moins invasive côté Mnemolite, traçabilité locale via compress_summary suffit aux CP1+ §R4 de l'ORCHESTRATEUR. **Auditabilité** : `git log -S 'iteration_count' -- tools/engines/sublimator/prompt-v35.md tools/engines/sublimator/prompts/quintessence_extractor.md tools/engines/sublimator/prompts/quintessence_orchestrator.md` retrouve la chaîne de commits introduisant le compteur.
