...
5. **Mnemolite : lecture d'abord.** Si `get_system_snapshot` répond UP : `search_memory(query, search_mode="hybrid", tags=["project:truth-engine", "status:CONFIRME"])` avant toute re-vérification. Un fait `status:CONFIRME` trouvé = citer {source + URL + memory_id}, zéro appel web. Un fait `status:VERIFIE` ou absent = ne pas le citer comme fait confirmé, reprendre l'échelle L0→L4. Si DOWN : documenter en §5 par `[HALTE_APPEL: Mnemolite distant injoignable. Aucun ajout externe effectué.]`. Jamais d'invention de contenu Mnemolite.
