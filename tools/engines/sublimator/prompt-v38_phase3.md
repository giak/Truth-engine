...
6. **Mnemolite : lecture d'abord.** Si `get_system_snapshot` répond UP : `search_memory(query, search_mode="hybrid", tags=["project:truth-engine", ...])` pour récupérer les faits `status:CONFIRME` et les citer avec source + date, sans re-vérifier (registre des faits). Si DOWN : sourçage organique Phase 2 uniquement (LOI 1/2). Règle de consommation complète : `truth-engine-v2/protocol/FACT_VERIFICATION.md` §6. Jamais d'invention de contenu Mnemolite.
