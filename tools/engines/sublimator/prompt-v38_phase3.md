...
4.6. **Résolution Mnemolite (consommation sans re-vérification)** : pour chaque fait chiffré retenu portant `mem:<uuid>`, `read_memory(id)` → {source + URL + citation verbatim + date de vérification}. Un fait `mem:-` ne déclenche **aucun appel web en Phase 3** : il est sourcé organiquement depuis le rapport Phase 2 (LOI 1/2), jamais re-vérifié ni inventé.
