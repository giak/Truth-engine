...
Modes :
  ...
  gate      check + certify, en une commande, sans revue LLM. Le verdict est
            ...
            notamment NO_UNCERTIFIED_FINAL, INSPECTED_TRACE_OK, PROVENANCE_ACCOUNTING_OK, QUERY_ACCOUNTING_OK
            et WRITEBACK_BLOCK_REASON_OK. En 2.10.6, `query target/actual` est interdit; SEARCH_ACTIVITY_V1 et SEMANTIC_COUNTS_V1 sont dérivés des registres runtime.
...
def _kernel_source_rows(text):
    """Parse canonical SRC rows with role, URL and explicit `fam:` provenance."""
    import re
    rows = {}
    duplicates = set()
    errors = []
    for raw in text.splitlines():
        ...
        if sid in rows:
            duplicates.add(sid)
            continue
        glyphs = [g for g in ("◈", "◉", "○") if g in line]
        fam_m = re.search(r"(?i)\bfam\s*:\s*((?:[A-E]|other:[a-z0-9][a-z0-9_-]{0,47})|—|-)(?=\s*\||\s|$)", line)
        urls = re.findall(r"https?://[^\s|]+", line)
        if len(glyphs) != 1:
            errors.append(f"SRC-{sid:03d}: rôle source ambigu/absent")
        if not fam_m:
            errors.append(f"SRC-{sid:03d}: fam: explicite absent")
            family = ""
        else:
            ...
            if family in ("-", "—"):
                family = ""
        if len(urls) > 1:
            errors.append(f"SRC-{sid:03d}: plusieurs URL dans la row")
        rows[sid] = {
            "role": glyphs[0] if len(glyphs) == 1 else "",
            "family": family,
            "url": _norm_url(urls[0]) if urls else "",
            "raw": raw.strip(),
        }
...
def _kernel_semantic_registry(text):
    ...
    for k,h in headings.items():
        ...
        for raw in sec.splitlines():
            ...
            if status in {"GAP", "UNKNOWN", "UNRESOLVED"} and not _gap_is_typed(payload):
                errors.append(f"{k}-{oid:03d}: GAP non typé ou sans description")
        observed[k]=len(ids)
...
def check_kernel_contract(deliverable, phase):
    ...
    # NARRATIVE_STABILITY_OK: prose is explicitly bounded and contains no
    ...
    if len(starts) != 1 or len(ends) != 1:
        ...
        if volatile:
            narrative_errors.append("IDs QRY volatils dans le récit: " + ",".join(volatile))
    if narrative_errors:
        results.append(("kernel:narrative_stability", "FAIL", "; ".join(narrative_errors)))
    else:
        results.append(("kernel:narrative_stability", "PASS", "récit borné et sans ID QRY volatil"))
