...
LEAD_REGISTRY:
  ID | SOURCE_ID | LOCATOR | LEAD | EVIDENCE_EXCERPT | KIND | MATERIALITY | ROUTES | LINKED_IDS | STATUS/GAP
...
Axis terminal semantics are KERNEL `NA_OK`, `GAP_OK`, `SAT_OK` and `TERM_AXS`; this file never weakens them.
...
```text
| ID | FROM | LINK_TYPE | MECHANISM | TO | SOURCE | COUNTER | STATUS |
ID = CAU-001...
```
